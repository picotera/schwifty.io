...
goog.provide('cast.games.common.receiver.ObjectPool');



/**
 * A generic object pool. Used instead of  {@code goog.structs.Pool} and
 * {@code goog.structs.SimplePool} because of simplicity and the fact that it
 * uses a single array and avoids resizing it, making it much more efficient on
 * low end receivers like ChromeCast.
 *
 ...
 * @template T
 */
cast.games.common.receiver.ObjectPool =
    function(name, initialSize, factory) {
 ...
 */
cast.games.common.receiver.ObjectPool.prototype.releaseObject =
    function(object) {
  ...
  var objectIndex = -1;
  for (var i = 0; i < this.currentIndex_; i++) {
  ...
  if (objectIndex != newObjectIndex) {
    var another = this.pool_[newObjectIndex];
